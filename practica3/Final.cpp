...
void animate(void)
{
	...
	if (animacion)
	{
		...
		if (estado == false) {
			if (movAuto_z >= 0.0f && movAuto_z<=200.0f) {
				movAuto_z += 3.0f;
			}
		}
...
int main()
{
	...
	// load models
	// -----------
	Model piso("resources/objects/pisopro/pispro.obj");
	...
	Model jeep("resources/objects/jeep/Jeep_Willys.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::scale(model, glm::vec3(5.0f));
		staticShader.setMat4("model", model);
		//staticShader.setVec3("dirLight.specular", glm::vec3(1.0f, 0.0f, 1.0f));
		...
		// -----------------------------------------------------------------
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(-450.0f, 50.0f,-400.0f));
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		...
		// ------------------------------------------------------
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(100.0f+posX, 10.0f, 100.0f+posZ));
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		//model = glm::scale(model, glm::vec3(7.50f));
		staticShader.setMat4("model", model);
