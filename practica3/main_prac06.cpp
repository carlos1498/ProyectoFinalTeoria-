...
/*------------- No. Cuenta     314000170             ---------------*/
//Evidencia Practica 6: enviar imagem de transparencia, cada cara del cubo una imagen distinta para formar un dado
//b) archivo de codigo donde se asigne imagen con transparencia y una cara del dado
...
//Camera
Camera camera(glm::vec3(0.0f, 0.0f, 15.0f));///posicion inicial de la camara
...
void LoadTextures()
{
	...
	t_stone = generateTextures("Texturas/piedracuad.jpg", 0);
	t_santa = generateTextures("Texturas/santafondo.png", 1);//valor de 1 porque tiene tranaparencia
	t_cube= generateTextures("Texturas/Cube04.jpg", 0);
}
...
void myData()
{
	...
	GLfloat verticesCubo[] = {
		...
		0.5f, -0.5f, -0.5f,		1.0f, 0.33f,	//V2 - Trasera
		-0.5f, -0.5f, -0.5f,	0.75f, 0.33f,	//V3
		-0.5f, 0.5f, -0.5f,		0.75f, 0.66f,	//V7
		0.5f, -0.5f, -0.5f,		1.0f, 0.33f,	//V2
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Practica 6 2022-2", NULL, NULL);
    if (window == NULL)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 1.0f, 1.0f, 0.0f);
		glBindTexture(GL_TEXTURE_2D, t_santa);//la variable de la imagen que va a ocupar como textura
		...
		glBindVertexArray(VAO[1]);
		modelOp = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -6.0f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", 1.0f, 0.0f, 1.0f);
